...
```
As an alternative, we might use a stack chart as an alternative representation that allows us to see the count in relation to other subreddits over time. 
```{r stackChart, eval=F}
ggplot(counted_df, aes(x=Group.1, y=x, fill=Group.2)) + 
    ylab("Number of Comments") +
    xlab("Date Comment Posted") +
    geom_area()
```
Both types of graph show the data over time, but they have different emphases in how they present the data.
...
```{r filterDate, eval=F}
filtered_date_df = reddit_df[reddit_df$Date > "2021-11-01" & reddit_df$Date < "2021-11-05",]
